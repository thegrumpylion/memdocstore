...
func newCollection(keyField string, keyFunc func(docstore.Document) interface{}, opts *Options) (driver.Collection, error) {
	if keyField == "" && keyFunc == nil {
		return nil, fmt.Errorf("must provide either keyField or keyFunc")
	}
	if opts == nil {
		opts = &Options{}
	}
...
func (c *collection) Key(doc driver.Document) (interface{}, error) {
	...
	if key == nil || driver.IsEmptyValue(reflect.ValueOf(key)) {
		return nil, fmt.Errorf("missing document key")
	}
	return key, nil
}
...
func (c *collection) runAction(ctx context.Context, a *driver.Action) error {
	...
	if !exists && (a.Kind == driver.Replace || a.Kind == driver.Update || a.Kind == driver.Get) {
		return fmt.Errorf("document with key %v does not exist", a.Key)
	}
	switch a.Kind {
	case driver.Create:
		// It is an error to attempt to create an existing document.
		if exists {
			return fmt.Errorf("Create: document with key %v exists", a.Key)
		}
		...
		if a.Key == nil {
			...
			if err := a.Doc.SetField(c.keyField, a.Key); err != nil {
				return fmt.Errorf("cannot set key field %q", c.keyField)
			}
		}
		fallthrough
	...
	default:
		return fmt.Errorf("unknown kind %v", a.Kind)
	}
	return nil
}
...
func add(x, y interface{}) (interface{}, error) {
	...
	case int64:
		...
		default:
			...
			return nil, fmt.Errorf("bad increment aount type %T", y)
		}
	case float64:
		switch y := y.(type) {
		case int64:
	...
	default:
		return nil, fmt.Errorf("value %v being incremented not int64 or float64", x)
	}
}
...
func (c *collection) checkRevision(arg driver.Document, current storedDoc) error {
	...
	wantRev, ok := r.(int64)
	if !ok {
		return fmt.Errorf("revision field %s is not an int64", c.opts.RevisionField)
	}
	if wantRev != curRev {
		return fmt.Errorf("mismatched revisions: want %d, current %d", wantRev, curRev)
	}
	return nil
}

// getAtFieldPath getsfmt.Errorf()the value of m at fp. It returns an error if fp is invalid
// (see getParentMap).
func getAtFieldPath(m map[string]interface{}, fp []string) (interface{}, error) {
	...
	return nil, fmt.Errorf("field %s not found", fp)
}
...
func getParentMap(m map[string]interface{}, fp []string, create bool) (map[string]interface{}, error) {
	...
	for _, k := range fp[:len(fp)-1] {
		...
		if !ok {
			return nil, fmt.Errorf("invalid field path %q at %q", strings.Join(fp, "."), k)
		}
	}
	return m, nil
}
...
func (c *collection) RevisionToBytes(rev interface{}) ([]byte, error) {
	r, ok := rev.(int64)
	if !ok {
		return nil, fmt.Errorf("revision %v of type %[1]T is not an int64", rev)
	}
